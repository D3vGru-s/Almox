...
async function carregarUsuariosSupabase() {
    ...
    const usuariosObj = {};
    data.forEach(u => {
        usuariosObj[u.nome.toLowerCase()] = {
            nome: u.nome,
            senha: '', // Senha não armazenada no Supabase neste exemplo
            permissoes: { entrada: true, saida: true, estoque: true, relatorios: true, usuarios: true, cadastrarProduto: true },
            perfil: { email: u.email || '', telefone: '', cargo: '', departamento: '', dataNascimento: '', foto: null }
        };
    });
    return usuariosObj;
}
...
class SistemaEstoque {
    constructor() {
        ...
        this.usuarios = {};
        carregarUsuariosSupabase().then(usuarios => {
    ...
    criarUsuario() {
        ...
        // Salvar no Supabase
        (async () => {
            const { error } = await supabase.from('usuarios').insert([{ nome, email: '' }]); // Ajuste se quiser coletar email do formulário
            if (error) {
                this.mostrarNotificacao('Erro ao salvar no Supabase: ' + error.message, 'error');
            } else {
                this.mostrarNotificacao('Usuário criado com sucesso no Supabase!', 'success');
                carregarUsuariosSupabase().then(usuarios => {
                    this.usuarios = usuarios;
                    this.atualizarTabelaUsuarios();
                });
